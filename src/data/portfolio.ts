export const personal = {
  firstName: "SIDIQ",
  lastName: "OLASODE",
  role: "Full Stack Developer & AI Enthusiast",
  email: "sidiqolasode@gmail.com",
  github: "https://github.com/sidiq20",
  githubHandle: "sidiq20",
  twitter: "https://x.com/tobiscrazy",
  linkedin: "https://www.linkedin.com/in/olasode-sidiq-290a382a6/",
};

export const projects = [
  {
    title: "Tungsten",
    category: "E-learning platform",
    year: "2026",
    description: "A comprehensive e-learning platform designed to provide seamless and interactive learning experiences for students and educators.",
    tech: ["TypeScript", "NestJS", "FastAPI", "Next.js", "Tailwind CSS", "PostgreSQL", "Redis", "GraphQL"],
    github: "#",
  },
  {
    title: "Trofida",
    category: "Proof of Discipline",
    year: "2026",
    description: "A solana / Axum based app for tracking your daily habits and goals",
    tech: ["TypeScript", "Solana / Rust", "Axum", "Next.js", "Tailwind CSS", "PostgreSQL", "GraphQL"],
    github: "#",
  },
  {
    title: "SevenEdge",
    category: "Organisation Website",
    year: "2026",
    description:
      "A scalable website for an organisation with a clean and modern interface",
    tech: ["TypeScript", "Next.js", "Clean Architecture", "FastAPI", "PostgreSQL"],
    github: "",
  },
  {
    title: "Stay Eko",
    category: "House Hosting Platform",
    year: "2024",
    description:
      "A comprehensive property management ecosystem consisting of mobile applications, a robust backend, and a modern web frontend.",
    tech: ["TypeScript", " FastAPI", "React Native", "MongoDB", "Redis"],
    github: "#",
  },
  {
    title: "Zikr on Sol",
    category: "Web3 & Blockchain",
    year: "2024",
    description:
      "High-frequency decentralized application built on the Solana blockchain, featuring optimized smart contract interactions.",
    tech: ["TypeScript", "Solana / Rust", "Anchor"],
    github: "#",
  },
  {
    title: "Whisper Exam",
    category: "Education Technology",
    year: "2025",
    description:
      "An AI-powered examination platform featuring automated proctoring and secure, encrypted data handling.",
    tech: ["Python", "Flask", "TypeScript", "TensorFlow", "Next.js"],
    github: "#",
  },
  {
    title: "LASUTH EMR",
    category: "Healthcare Tech",
    year: "2023",
    description:
      "A secure, encrypted dashboard for managing patient information and streamlining clinical workflows in hospital environments.",
    tech: ["Next.js", "Firebase", "Encryption"],
    github: "#",
  },
];

export const stack = [
  "React / Next.js",
  "TypeScript",
  "Python",
  "FastAPI / Flask",
  "Rust / Solana",
  "Rust / Axum",
  "TensorFlow",
  "Docker",
  "PostgreSQL",
  "MongoDB",
  "Firebase",
  "GraphQL",
  "Git",
  "Ubuntu",
  "Selenium",
  "Postman"
];

export const about = {
  statement:
    "I design and build digital products at the intersection of code and craft.",
  body: "Focused on building intuitive interfaces and robust systems — combining modern web technologies with AI and blockchain to create products that feel as good as they look. Specializing in React, Next.js, Python, and the Solana ecosystem. Open to work.",
};
